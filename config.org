...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package esup
    ...
    (setq esup-user-init-file
          (expand-file-name (concat (file-name-base y/user-init-config) ".el")
...
* Common
- basic routines for config
- basic packages for config
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-minor-mode y/read-only-keybind-mode "Read-only minor keybind"
    ...
    (let ((map (make-sparse-keymap)))
      ...
      (define-key map (kbd "TAB") #'sp-forward-symbol)
      ;; (define-key map (kbd "<backtab>") #'sp-backward-symbol)
      ;; (define-key map (kbd "RET") #'y/tags-jump-symbol)
      ;; (define-key map (kbd "DEL") #'xref-pop-marker-stack)
      map))
...
** publish
A big section. See [[#publish][Publish]].
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun asm-calculate-indentation ()
    (or
     ...
     ;; (or (indent-next-tab-stop 0)) ;; This line is original one
     (or 4)))
#+END_SRC
** c/c++ style
...
#+begin_src emacs-lisp
  ...
  (use-package counsel-gtags
    :diminish
    :init
    (advice-add 'counsel-gtags-dwim :before
                #'(lambda()
...
** html
   :PROPERTIES:
   :CUSTOM_ID: publish-config
   :END:
DO NOT modify the custom-id publish-config, The batch scripts use it to load
elisp snippet properly.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (setq org-html-postamble (y:file-string "misc/publish-disqus.html"))
  ...
  (setq org-publish-project-alist
        `(("blogs"
          ...
          ("main"
           ...
           :with-toc t)
          ("papers"
           :base-directory "~/org/h/papers"
           :base-extension "org"
           :publishing-directory "~/hp/papers/"
           :recursive nil
           ...
           :auto-sitemap t
           :auto-sitemap t
           :sitemap-function nil
           :sitemap-format-entry nil
           ...
           :with-toc t)
          ("sites"
           :components ("attachment" "main" "blogs" "papers"))))
...
* Deep Customize
#+BEGIN_SRC emacs-lisp
  (defun y/replace-ligatures()
    "Execute function to replace ligatures ff, ffi, fi."
    (interactive)
    (save-excursion
      (let ((c 0))
        (dolist (patn '(("ﬃ" . "ffi") ("ﬂ" . "fl") ("ﬁ" . "fi")
                        ("ﬀ" . "ff") ("ﬄ" . "ffl") ("’" . "'")
                        ("‘‘" . "\"") ("’’" . "\"") ("''" . "\"")))
          (goto-char (point-min))
          (let* ((s (car patn))
            ...
            (while (re-search-forward s nil t)
              (setq cc (+ 1 cc))
              (replace-match d nil nil))
            (setq c (+ c cc))
            (message "Replace ligatures %2d: %s --> %s" cc s d)))
        (message "Replaces ligatures count: %d" c))))

  (global-set-key (kbd "C-x C-x r") #'y/replace-ligatures)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (when (boundp 'y/time-begin-seconds)
    ...
    (message "y/time-elapsed-time is %.3f seconds (file: %s and %s)"
             ...
             user-init-file y/user-init-config)
